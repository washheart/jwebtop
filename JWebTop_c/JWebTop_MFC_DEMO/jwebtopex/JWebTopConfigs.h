#ifndef CEF_JWEBTOP_CONFIGS_ex_H_
#define CEF_JWEBTOP_CONFIGS_ex_H_
#include <Windows.h>
#include <string>
class
	JWebTopConfigs{
public:
	...
	std::wstring appDefFile;          // 当前配置对应的配置文件（有可能为null）
	std::wstring appPath;             // 当前配置文件所在目录（不能为null）
	std::wstring url=L"file:///index.html";   // 要打开的链接地址
	std::wstring appendJs;			   // 通过此参数，可以附加一个js到当前页面（在OnLoad中通过<script type='text/javascript' src='appendJs.js'></script>)附加）
	std::wstring name;                // 窗口名称
	std::wstring icon;                // 窗口图标

	// [CEF]小节：cef配置参数（cef相关参数只在应用启动时起作用），配置在[CEF]小节下
	int single_process = 1;                                 // 是否使用单进程模式：JWebTop默认使用。CEF默认不使用单进程模式
	std::wstring user_data_path = std::wstring(L"data/jwebtop");  // 用户数据保存目录
	std::wstring cache_path = std::wstring(L"data/jwebtop");      // 浏览器缓存数据的保存目录
	int persist_session_cookies;                            // 是否需要持久化用户cookie数据（若要设置为true，需要同时指定cache_path）
	std::wstring user_agent = std::wstring(L"JWebTop2.0");        // HTTP请求中的user_agent,CEF默认是Chorminum的user agent
	std::wstring locale = std::wstring(L"zh-CN");                 // CEF默认是en-US
	int log_severity = LOGSEVERITY_DISABLE;                 // 指定日志输出级别，取值[0,1,2,3,4,99]默认不输出(99)
	std::wstring log_file;                                     // 指定调试时的日志文件，默认为"debug.log"。如果关闭日志，则不输出日志
	std::wstring resources_dir_path;                           // 指定cef资源文件（ cef.pak、devtools_resources.pak）的目录，默认从程序运行目录取
	std::wstring locales_dir_path;                             // 指定cef本地化资源(locales)目录，默认去程序运行目录下的locales目录
	int ignore_certificate_errors;                          // 是否忽略SSL证书错误
	int remote_debugging_port;                              // 远程调试端口，取值范围[1024-65535]

	std::wstring proxyServer;									// 设置代理服务器地址。设置一个http代理服务器：  设置多个代理服务器：--proxy-server="https=proxy1:80;http=socks4://baz:1080"
	std::wstring proxyAuthUser;								// 登录代理服务器时需要的用户名（注意：不管proxyServer设置了多少代理服务器，这里暂时值支持一组用户名和密码）
	std::wstring proxyAuthPwd;									// 登录代理服务器时需要的密码

	int no_sandbox = 1;                                     // 是否使用沙盒模式：JWebTop默认不使用。CEF默认使用。此参数在JWebTop中不可以配置为使用，否则会造成部分JWebTop JS不能使用

	
	///// 以下cef参数暂不支持配置
	//std::wstring product_version;                 // 如果指定了user_agent，此参数会被忽略,CEF默认是Chorminum的版本号
	//int pack_loading_disabled;// 禁止从resources_dir_path和locales_dir_path读取数据，转为从CefApp::GetResourceBundleHandler()获取
	//std::wstring browser_subprocess_path;
