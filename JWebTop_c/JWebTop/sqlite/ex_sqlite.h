	...
	namespace db {
		...
		*/
		...
		public:
			bool Execute(const CefString& name, CefRefPtr<CefV8Value> object, const CefV8ValueList& arguments, CefRefPtr<CefV8Value>& retval, CefString& exception);
		private:
			typedef CefRefPtr<CefV8Value>(JJH_DB_query::* outfun)(CefV8ValueList args);// 定义一个函数指针，用户输出数据库的某列数据到js
			...
			inline bool outputOneRowCol(CefRefPtr<CefV8Value>(JJH_DB_query::* outfun)(CefV8ValueList));
